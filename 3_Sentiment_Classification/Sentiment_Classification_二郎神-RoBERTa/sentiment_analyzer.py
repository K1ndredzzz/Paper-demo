...
class StockSentimentAnalyzer:
    def __init__(self, model_id='Fengshenbang/Erlangshen-RoBERTa-330M-Sentiment'):
        """
        初始化情感分析器
        
        Args:
            model_id (str): ModelScope模型ID
                默认使用 'Fengshenbang/Erlangshen-RoBERTa-330M-Sentiment'
        """
        self.semantic_cls = pipeline(Tasks.text_classification, model_id)
        # 情感标签映射 (负面:-1, 正面:1)
        self.label_map = {"负向": -1, "正向": 1}
            ...
            if 'scores' in result and 'labels' in result:
                ...
                if positive_idx != -1 and negative_idx != -1:
                    ...
                    if positive_prob > negative_prob:
                        polarity = self.label_map["正向"]  # 1
                    else:
                        polarity = self.label_map["负向"]  # -1
