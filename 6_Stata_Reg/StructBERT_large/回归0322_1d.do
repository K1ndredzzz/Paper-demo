...
reg2docx FE_Entity FE_Entity_Time using Regression_1d.docx,replace b(%12.4f)t(%12.4f)drop(*.time)scalars(N r2 F)title(PanelReg_1)note(***p<0.01，**p<0.05，*p<0.10)

/*
//超前1期
xtreg F.forward_ret_1d forward_ret_1d avg_sentiment sentiment_std avg_intensity comment_count sentiment_consensus F.close F.volume F.amount F.amplitude F.pct_change F.price_change F.turnover_rate i.time, fe
est store FE_Entity_Time_1
//超前2期
xtreg F2.forward_ret_1d L1.forward_ret_1d forward_ret_1d avg_sentiment sentiment_std avg_intensity comment_count sentiment_consensus F2.close F2.volume F2.amount F2.amplitude F2.pct_change F2.price_change F2.turnover_rate i.time, fe
est store FE_Entity_Time_2
//超前3期
xtreg F3.forward_ret_1d F2.forward_ret_1d L1.forward_ret_1d forward_ret_1d avg_sentiment sentiment_std avg_intensity comment_count sentiment_consensus F3.close F3.volume F3.amount F3.amplitude F3.pct_change F3.price_change F3.turnover_rate i.time, fe
